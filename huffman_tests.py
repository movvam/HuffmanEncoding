...
class TestList(unittest.TestCase):
   ...
   def test_onechar_encode(self):
      huffman_encode("onechar.txt","encoded_onechar.txt")
      self.assertTrue(filecmp.cmp("encoded_onechar.txt", "onechar_soln.txt"))

   def test_morechar_encode(self):
      huffman_encode("morechar.txt","encoded_morechar.txt")
      self.assertTrue(filecmp.cmp("encoded_morechar.txt", "morechar_soln.txt"))
   ...
   def test_preord(self):
      freqlist = cnt_freq("file1.txt")
      hufftree = create_huff_tree(freqlist)
      string = tree_preord(hufftree)
      self.assertEqual(string, "0000a2f2b4c8d16")
